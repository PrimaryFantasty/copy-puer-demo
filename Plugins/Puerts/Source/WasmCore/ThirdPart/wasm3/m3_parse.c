...
M3Result ParseSection_Import(IM3Module io_module, bytes_t i_bytes, cbytes_t i_end)
{
    ...
    for (u32 i = 0; i < numImports; ++i)
    {
        ...
        m3log(parse, "    kind: %d '%s.%s' ", (u32) importKind, import.moduleUtf8, import.fieldUtf8);
        switch (importKind)
        {
    ...
    for (u32 f = 0; f < numFunctions; ++f)
    {
        ...
        if (size)
        {
            ...
            if (i_bytes <= i_end)
            {
                /*
                u32 numLocalBlocks;
_               (ReadLEB_u32 (& numLocalBlocks, & ptr, i_end));                                      m3log (parse, "    code size:
%-4d", size);
...
_                   (NormalizeType (& normalType, wasmType));
                    ...
                    numLocals += varCount;                                                      m3log (parse, "      %2d locals;
type: '%s'", varCount, c_waTypes [normalType]);
                }
                 */
...
M3Result ParseSection_Data(M3Module* io_module, bytes_t i_bytes, cbytes_t i_end)
{
    ...
    for (u32 i = 0; i < numDataSegments; ++i)
    {
        ...
        m3log(parse, "    segment [%u]  memory: %u;  expr-size: %d;  size: %d", i, segment->memoryRegion, segment->initExprSize,
            segment->size);
