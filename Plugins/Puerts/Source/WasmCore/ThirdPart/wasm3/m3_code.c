...
IM3CodePage NewCodePage(IM3Runtime i_runtime, u32 i_minNumLines)
{
    ...
    if (page)
    {
        ...
        m3log(runtime, "new page: %p; seq: %d; bytes: %d; lines: %d", GetPagePC(page), page->info.sequence, pageSize,
            page->info.numLines);
    }
...
void FreeCodePages(IM3CodePage* io_list)
{
    ...
    {
        m3log(
            code, "free page: %d; %p; util: %3.1f%%", page->info.sequence, page, 100. * page->info.lineIndex / page->info.numLines);
