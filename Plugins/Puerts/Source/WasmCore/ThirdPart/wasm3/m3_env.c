...
IM3Runtime m3_NewRuntime(IM3Environment i_environment, u32 i_stackSizeInBytes, void* i_userdata)
{
    ...
    if (runtime)
    {
        ...
        if (runtime->stack)
        {
            ...
            m3log(runtime, "new stack: %p", runtime->stack);
        }
        else
            m3_Free(runtime);
    }
...
M3Result ResizeMemory(IM3Runtime io_runtime, u32 i_numPages)
{
    ...
    if (numPagesToAlloc <= memory->maxPages)
    {
        ...
        m3log(runtime, "resized old: %p; mem: %p; length: %zu; pages: %d", oldMallocated, memory->mallocated,
            memory->mallocated->length, memory->numPages);
    }
    else
...
M3Result InitGlobals(IM3Module io_module)
{
    ...
    if (io_module->numGlobals)
    {
        ...
        //          else result = ErrorModule (m3Err_mallocFailed, io_module, "could allocate globals for module: '%s",
        //          io_module->name);
    }
...
void ReleaseCodePage(IM3Runtime i_runtime, IM3CodePage i_codePage)
{
    if (i_codePage)
    {
        ...
        m3log(runtime, "runtime: %p; open-pages: %d; full-pages: %d; active: %d; total: %d", i_runtime, numOpen, numFull,
            i_runtime->numActiveCodePages, i_runtime->numCodePages);
...
M3Result m3Error(M3Result i_result, IM3Runtime i_runtime, IM3Module i_module, IM3Function i_function, const char* const i_file,
