...
#define ErrorCompile(RESULT, COMP, FORMAT, ...) \
    _m3Error(RESULT, COMP->runtime, COMP->module, NULL, __FILE__, __LINE__, FORMAT, ##__VA_ARGS__)
...
#define m3_ReallocArray(STRUCT, PTR, NEW, OLD) \
    (STRUCT*) m3_Realloc_Impl((void*) (PTR), sizeof(STRUCT) * (NEW), sizeof(STRUCT) * (OLD))
#define m3_Free(P)                 \
    do                             \
    {                              \
        m3_Free_Impl((void*) (P)); \
...
void ReportError(
    IM3Runtime io_runtime, IM3Module i_module, IM3Function i_function, ccstr_t i_errorMessage, ccstr_t i_file, u32 i_lineNum);
