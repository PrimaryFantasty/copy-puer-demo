...
static M3_NOINLINE M3Result EnsureCodePageNumLines(IM3Compilation o, u32 i_numLines)
{
    ...
    if (NumFreeLines(o->page) < i_numLines)
    {
        ...
        if (page)
        {
            m3log(emit, "bridging new code page from: %d %p (free slots: %d) to: %d", o->page->info.sequence, GetPC(o),
                NumFreeLines(o->page), page->info.sequence);
...
static M3_NOINLINE pc_t EmitPointer(IM3Compilation o, const void* const i_pointer)
{
...
static const IM3Operation c_preserveSetSlot[] = {
    NULL, op_PreserveSetSlot_i32, op_PreserveSetSlot_i64, FPOP(op_PreserveSetSlot_f32), FPOP(op_PreserveSetSlot_f64)};
static const IM3Operation c_setSetOps[] = {NULL, op_SetSlot_i32, op_SetSlot_i64, FPOP(op_SetSlot_f32), FPOP(op_SetSlot_f64)};
static const IM3Operation c_setGlobalOps[] = {
    NULL, op_SetGlobal_i32, op_SetGlobal_i64, FPOP(op_SetGlobal_f32), FPOP(op_SetGlobal_f64)};
static const IM3Operation c_setRegisterOps[] = {
    NULL, op_SetRegister_i32, op_SetRegister_i64, FPOP(op_SetRegister_f32), FPOP(op_SetRegister_f64)};
...
static inline bool IsConstantSlot(IM3Compilation o, u16 i_slot)
{
    return (i_slot >= o->slotFirstConstIndex and i_slot < o->slotMaxConstIndex);
}
static inline bool IsSlotAllocated(IM3Compilation o, u16 i_slot)
{
...
static inline u16 GetNumBlockValuesOnStack(IM3Compilation o)
{
    return o->stackIndex - o->block.blockStackIndex;
}

static inline bool IsStackTopInRegister(IM3Compilation o)
{
    return IsStackIndexInRegister(o, (i32) GetStackTopIndex(o));
}
static inline bool IsStackTopMinus1InRegister(IM3Compilation o)
{
    return IsStackIndexInRegister(o, (i32) GetStackTopIndex(o) - 1);
}
static inline bool IsStackTopMinus2InRegister(IM3Compilation o)
{
    return IsStackIndexInRegister(o, (i32) GetStackTopIndex(o) - 2);
}

static inline bool IsStackTopInSlot(IM3Compilation o)
{
    return not IsStackTopInRegister(o);
}

static inline bool IsValidSlot(u16 i_slot)
{
...
static M3Result Compile_SetLocal(IM3Compilation o, m3opcode_t i_opcode)
{
    ...
    if (localIndex < GetFunctionNumArgsAndLocals(o->function))
    {
        ...
        u16 preserveSlot;
        _(FindReferencedLocalWithinCurrentBlock(
            o, &preserveSlot, localSlot));    // preserve will be different than local, if referenced
...
static M3Result Compile_Call(IM3Compilation o, m3opcode_t i_opcode)
{
    _try
    {
        ...
        if (function)
        {
            m3log(compile, d_indent " (func= [%d] '%s'; args= %d)", get_indention_string(o), functionIndex,
                m3_GetFunctionName(function), function->funcType->numArgs);
            if (function->module)
            {
                u16 slotTop;
            ...
            {
                _throw(ErrorCompile(m3Err_functionImportMissing, o, "'%s.%s'", GetFunctionImportModuleName(function),
...
static M3Result PreserveArgsAndLocals(IM3Compilation o)
{
    ...
    if (o->stackIndex > o->stackFirstDynamicIndex)
    {
        ...
        for (u32 i = 0; i < numArgsAndLocals; ++i)
        {
            ...
            {
                u8 type = GetStackTypeFromBottom(o, i);
                d_m3Assert(type != c_m3Type_none) IM3Operation op = Is64BitType(type) ? op_CopySlot_64 : op_CopySlot_32;
...
static M3Result Compile_Select(IM3Compilation o, m3opcode_t i_opcode)
{
    ...
    if (IsFpType(type))
    {
        ...
        if (not IsStackTopMinus1InRegister(o) and not IsStackTopMinus2InRegister(o))
        {
            _(PreserveRegisterIfOccupied(o, type));
        }
    ...
    else if (IsIntType(type))
    {
        ...
        if (not IsStackTopInRegister(o) and not IsStackTopMinus1InRegister(o) and not IsStackTopMinus2InRegister(o))
        {
            _(PreserveRegisterIfOccupied(o, type));
        }
...
#define d_logOp(OP)               \
    {                             \
        op_##OP, NULL, NULL, NULL \
    }
#define d_logOp2(OP1, OP2)             \
    {                                  \
        op_##OP1, op_##OP2, NULL, NULL \
    }

#define d_emptyOpList          \
    {                          \
        NULL, NULL, NULL, NULL \
    }
#define d_unaryOpList(TYPE, NAME)                                  \
    {                                                              \
        op_##TYPE##_##NAME##_r, op_##TYPE##_##NAME##_s, NULL, NULL \
    }
#define d_binOpList(TYPE, NAME)                                                         \
    {                                                                                   \
        op_##TYPE##_##NAME##_rs, op_##TYPE##_##NAME##_sr, op_##TYPE##_##NAME##_ss, NULL \
    }
#define d_storeFpOpList(TYPE, NAME)                                                                        \
    {                                                                                                      \
        op_##TYPE##_##NAME##_rs, op_##TYPE##_##NAME##_sr, op_##TYPE##_##NAME##_ss, op_##TYPE##_##NAME##_rr \
    }
#define d_commutativeBinOpList(TYPE, NAME)                           \
    {                                                                \
        op_##TYPE##_##NAME##_rs, NULL, op_##TYPE##_##NAME##_ss, NULL \
    }
#define d_convertOpList(OP)                                        \
    {                                                              \
        op_##OP##_r_r, op_##OP##_r_s, op_##OP##_s_r, op_##OP##_s_s \
    }
...
#define d_m3DebugTypedOp(OP) M3OP(#OP, 0, none, {op_##OP##_i32, op_##OP##_i64})
    ...
    d_m3DebugOp(Compile), d_m3DebugOp(Entry), d_m3DebugOp(End), d_m3DebugOp(Unsupported), d_m3DebugOp(CallRawFunction),
...
#if d_m3HasFloat
    d_m3DebugOp(Select_f32_sss), d_m3DebugOp(Select_f32_srs), d_m3DebugOp(Select_f32_ssr), d_m3DebugOp(Select_f32_rss),
    d_m3DebugOp(Select_f32_rrs), d_m3DebugOp(Select_f32_rsr),

    d_m3DebugOp(Select_f64_sss), d_m3DebugOp(Select_f64_srs), d_m3DebugOp(Select_f64_ssr), d_m3DebugOp(Select_f64_rss),
    d_m3DebugOp(Select_f64_rrs), d_m3DebugOp(Select_f64_rsr),
#endif
...
M3Result CompileFunction(IM3Function io_function)
{
    ...
    m3log(compile, "compiling: [%d] %s %s; wasm-size: %d", io_function->index, m3_GetFunctionName(io_function),
        SPrintFuncTypeSignature(funcType), (u32) (io_function->wasmEnd - io_function->wasm));
    IM3Runtime runtime = io_function->module->runtime;

    IM3Compilation o = &runtime->compilation;
    d_m3Assert(
        d_m3MaxFunctionSlots >= d_m3MaxFunctionStackHeight * (d_m3Use32BitSlots + 1))    // need twice as many slots in 32-bit mode
    ...
    _try
    {
        ...
        m3log(compile, "start stack index: %d", (u32) o->stackFirstDynamicIndex);
        _(EmitOp(o, op_Entry));
        ...
        m3log(compile, "unique constant slots: %d; unused slots: %d", numConstantSlots,
            o->slotFirstDynamicIndex - o->slotMaxConstIndex);
