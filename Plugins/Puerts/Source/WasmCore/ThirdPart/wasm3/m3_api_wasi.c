...
#elif defined(__wasi__) || defined(__APPLE__) || defined(__ANDROID_API__) || defined(__OpenBSD__) || defined(__linux__) || \
    defined(__EMSCRIPTEN__) || defined(__CYGWIN__)
#include <unistd.h>
#include <sys/uio.h>
...
static inline const void* copy_iov_to_host(
    IM3Runtime runtime, void* _mem, struct iovec* host_iov, wasi_iovec_t* wasi_iov, int32_t iovs_len)
{
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, fd) m3ApiGetArgMem(wasi_iovec_t*, wasi_iovs)
...
M3Result m3_LinkWASI(IM3Module module)
{
    ...
    //_ (SuppressLookupFailure (m3_LinkRawFunction (module, "wasi_unstable",          "fd_filestat_get",   "i(i*)",
    //&m3_wasi_unstable_fd_filestat_get))); _ (SuppressLookupFailure (m3_LinkRawFunction (module, "wasi_snapshot_preview1",
    //"fd_filestat_get",   "i(i*)",     &m3_wasi_snapshot_preview1_fd_filestat_get))); _ (SuppressLookupFailure (m3_LinkRawFunction
    //(module, "wasi_unstable",          "path_filestat_get", "i(ii*i*)",  &m3_wasi_unstable_path_filestat_get))); _
    //(SuppressLookupFailure (m3_LinkRawFunction (module, "wasi_snapshot_preview1", "path_filestat_get", "i(ii*i*)",
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        //_     (SuppressLookupFailure (m3_LinkRawFunction (module, wasi, "poll_oneoff",          "i(**i*)",
        //&m3_wasi_generic_poll_oneoff)));
