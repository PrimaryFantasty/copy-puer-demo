...
const char* wasi_whence2str(uvwasi_whence_t whence)
{
    switch (whence)
    {
        case UVWASI_WHENCE_SET:
            return "SET";
        case UVWASI_WHENCE_CUR:
            return "CUR";
        case UVWASI_WHENCE_END:
            return "END";
        default:
            return "<unknown>";
    }
...
#define WASI_TRACE(fmt, ...)                                          \
    {                                                                 \
        fprintf(stderr, "%s " fmt, __FUNCTION__ + 16, ##__VA_ARGS__); \
        fprintf(stderr, " => %s\n", wasi_errno2str(ret));             \
    }
#else
#define WASI_TRACE(fmt, ...)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArgMem(uvwasi_size_t*, argc) m3ApiGetArgMem(uvwasi_size_t*, argv_buf_size)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArgMem(uvwasi_size_t*, env_count) m3ApiGetArgMem(uvwasi_size_t*, env_buf_size)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(char*, path) m3ApiGetArg(uvwasi_size_t, path_len)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(uint8_t*, buf)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArg(uvwasi_fdflags_t, flags)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArg(uvwasi_rights_t, rights_base)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArg(uvwasi_filesize_t, size)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArg(uvwasi_timestamp_t, atim)
        m3ApiGetArg(uvwasi_timestamp_t, mtim) m3ApiGetArg(uvwasi_fstflags_t, fst_flags)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArg(uvwasi_filedelta_t, offset)
        m3ApiGetArg(uint32_t, wasi_whence) m3ApiGetArgMem(uvwasi_filesize_t*, result)
    ...
    WASI_TRACE("fd:%d, offset:%" PRIu64 ", whence:%s | result:%" PRIu64, fd, offset, wasi_whence2str(whence), pos);
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, from) m3ApiGetArg(uvwasi_fd_t, to)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(uvwasi_filesize_t*, result)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(const char*, path) m3ApiGetArg(uvwasi_size_t, path_len)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(const char*, path) m3ApiGetArg(uvwasi_size_t, path_len)
        m3ApiGetArgMem(char*, buf) m3ApiGetArg(uvwasi_size_t, buf_len) m3ApiGetArgMem(uvwasi_size_t*, bufused)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, old_fd) m3ApiGetArgMem(const char*, old_path)
        m3ApiGetArg(uvwasi_size_t, old_path_len) m3ApiGetArg(uvwasi_fd_t, new_fd) m3ApiGetArgMem(const char*, new_path)
            m3ApiGetArg(uvwasi_size_t, new_path_len)

                m3ApiCheckMem(old_path, old_path_len);
    m3ApiCheckMem(new_path, new_path_len);

    uvwasi_errno_t ret = uvwasi_path_rename(&uvwasi, old_fd, old_path, old_path_len, new_fd, new_path, new_path_len);
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArgMem(const char*, old_path) m3ApiGetArg(uvwasi_size_t, old_path_len)
        m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(const char*, new_path) m3ApiGetArg(uvwasi_size_t, new_path_len)

            m3ApiCheckMem(old_path, old_path_len);
    m3ApiCheckMem(new_path, new_path_len);

    uvwasi_errno_t ret = uvwasi_path_symlink(&uvwasi, old_path, old_path_len, fd, new_path, new_path_len);
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, dirfd) m3ApiGetArg(uvwasi_lookupflags_t, dirflags)
        m3ApiGetArgMem(const char*, path) m3ApiGetArg(uvwasi_size_t, path_len) m3ApiGetArg(uvwasi_oflags_t, oflags)
            m3ApiGetArg(uvwasi_rights_t, fs_rights_base) m3ApiGetArg(uvwasi_rights_t, fs_rights_inheriting)
                m3ApiGetArg(uvwasi_fdflags_t, fs_flags) m3ApiGetArgMem(uvwasi_fd_t*, fd)
    ...
    uvwasi_errno_t ret =
        uvwasi_path_open(&uvwasi, dirfd, dirflags, path, path_len, oflags, fs_rights_base, fs_rights_inheriting, fs_flags, &uvfd);

    WASI_TRACE(
        "dirfd:%d, dirflags:0x%x, path:%s, oflags:0x%x, fs_flags:0x%x | fd:%d", dirfd, dirflags, path, oflags, fs_flags, uvfd);
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArg(uvwasi_lookupflags_t, flags)
        m3ApiGetArgMem(const char*, path) m3ApiGetArg(uint32_t, path_len) m3ApiGetArgMem(uint8_t*, buf)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(wasi_iovec_t*, wasi_iovs)
        m3ApiGetArg(uvwasi_size_t, iovs_len) m3ApiGetArg(uvwasi_filesize_t, offset) m3ApiGetArgMem(uvwasi_size_t*, nread)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(wasi_iovec_t*, wasi_iovs)
        m3ApiGetArg(uvwasi_size_t, iovs_len) m3ApiGetArgMem(uvwasi_size_t*, nread)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(wasi_iovec_t*, wasi_iovs)
        m3ApiGetArg(uvwasi_size_t, iovs_len) m3ApiGetArgMem(uvwasi_size_t*, nwritten)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(wasi_iovec_t*, wasi_iovs)
        m3ApiGetArg(uvwasi_size_t, iovs_len) m3ApiGetArg(uvwasi_filesize_t, offset) m3ApiGetArgMem(uvwasi_size_t*, nwritten)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArgMem(void*, buf) m3ApiGetArg(uvwasi_size_t, buf_len)
        m3ApiGetArg(uvwasi_dircookie_t, cookie) m3ApiGetArgMem(uvwasi_size_t*, bufused)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd) m3ApiGetArg(uvwasi_filesize_t, offset)
        m3ApiGetArg(uvwasi_filesize_t, length) m3ApiGetArg(uvwasi_advice_t, advice)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArgMem(uint8_t*, buf) m3ApiGetArg(uvwasi_size_t, buf_len)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_clockid_t, wasi_clk_id) m3ApiGetArgMem(uvwasi_timestamp_t*, resolution)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_clockid_t, wasi_clk_id) m3ApiGetArg(uvwasi_timestamp_t, precision)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArgMem(const uvwasi_subscription_t*, in) m3ApiGetArgMem(uvwasi_event_t*, out)
        m3ApiGetArg(uvwasi_size_t, nsubscriptions) m3ApiGetArgMem(uvwasi_size_t*, nevents)
...
{
    m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_signal_t, sig)
...
{
    m3ApiReturnType(uint32_t) uvwasi_errno_t ret = uvwasi_sched_yield(&uvwasi);
...
M3Result m3_LinkWASIWithOptions(IM3Module module, uvwasi_options_t init_options)
{
    ...
    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, "wasi_snapshot_preview1", "fd_seek", "i(iIi*)", &m3_wasi_snapshot_preview1_fd_seek)));
    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, "wasi_unstable", "fd_filestat_get", "i(i*)", &m3_wasi_unstable_fd_filestat_get)));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, "wasi_snapshot_preview1", "fd_filestat_get", "i(i*)", &m3_wasi_snapshot_preview1_fd_filestat_get)));
    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, "wasi_unstable", "path_filestat_get", "i(ii*i*)", &m3_wasi_unstable_path_filestat_get)));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, "wasi_snapshot_preview1", "path_filestat_get", "i(ii*i*)", &m3_wasi_snapshot_preview1_path_filestat_get)));
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        _(SuppressLookupFailure(
            m3_LinkRawFunctionEx(module, wasi, "args_sizes_get", "i(**)", &m3_wasi_generic_args_sizes_get, wasi_context)));
        ...
        _(SuppressLookupFailure(
            m3_LinkRawFunction(module, wasi, "environ_sizes_get", "i(**)", &m3_wasi_generic_environ_sizes_get)));
        ...
        _(SuppressLookupFailure(
            m3_LinkRawFunction(module, wasi, "fd_fdstat_set_flags", "i(ii)", &m3_wasi_generic_fd_fdstat_set_flags)));
        _(SuppressLookupFailure(
            m3_LinkRawFunction(module, wasi, "fd_fdstat_set_rights", "i(iII)", &m3_wasi_generic_fd_fdstat_set_rights)));
        _(SuppressLookupFailure(
            m3_LinkRawFunction(module, wasi, "fd_filestat_set_size", "i(iI)", &m3_wasi_generic_fd_filestat_set_size)));
        _(SuppressLookupFailure(
            m3_LinkRawFunction(module, wasi, "fd_filestat_set_times", "i(iIIi)", &m3_wasi_generic_fd_filestat_set_times)));
        ...
        _(SuppressLookupFailure(
            m3_LinkRawFunction(module, wasi, "path_create_directory", "i(i*i)", &m3_wasi_generic_path_create_directory)));
        ...
        _(SuppressLookupFailure(
            m3_LinkRawFunction(module, wasi, "path_remove_directory", "i(i*i)", &m3_wasi_generic_path_remove_directory)));
